...
class Delicious {
	...
	// L'idée est de fournir les fonctions qui vont faire appel à ces méthodes.
	// L'API Delicious renvoie soit un code d'erreur, soit dans le cas d'un succès, du contenu au format xml.
	// Ce contenu devra etre écrit dans un fichier xml
	// Il ne reste plus qu'à parser le contenu de ce fichier pour afficher les éléments souhaités.
	//
	//
	// Voici quelques consignes données par Delicious.
	// Vous devrez en tenir compte dans les développement que vous réaliserez en utilisant cette librairie.
...
/********************************************************************************************************************************************************/
	...
	function renameTags($old,$new) {
		$method = "/tags/rename?old=$old&new=$new";
...
/********************************************************************************************************************************************************/
	...
	// Arguments
	//		(optional) bundle : Fetch just the named bundle.
	function allBundles() {
		$method = "/tags/bundles/all?";
		
		if($this->bundle != NULL)
			$method .= "bundle=".$this->bundle;
	...
	// Arguments
	//		(required) bundle : name of the bundle
	//		(required) tags : list of tags, space-separated
	//			Ex. : &tags={TAG}+{TAG}+...+{TAG}
	// Examples Response
	//		<result>ok</result>
	//		<result>you must supply a bundle name and at least one tag</result>
	function setBundles() {
		$method = "/tags/bundles/set?";
		
		if($this->bundle != NULL)
			$method .= "bundle=".$this->bundle."&";
		
		if($this->tag != NULL)
			$method .= "tags=".$this->tag;
